...
class Sim {
  ...
  /// Rewind to a snapshot taken earlier in this run (second chance). Flips
  /// after the snapshot frame are dropped so the replay stays consistent.
  void resumeFrom(SimState snapshot) {
    s = snapshot.copy();
    flips.removeWhere((f) => f > s.frame);
  }
