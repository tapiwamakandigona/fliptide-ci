...
/// CLEARED card: the first star lands this long after the card appears …
const Duration kStarRevealDelay = Duration(milliseconds: 350);

/// … and each further star this much later (the chimes follow the same beat).
const Duration kStarRevealStep = Duration(milliseconds: 170);

class StarRow extends StatelessWidget {
  const StarRow({super.key, required this.earned, this.size = 16, this.animate = false});
  final int earned;
  final double size;

  /// Earned stars pop in one after another (elastic scale + fade). Off in
  /// lists; on for the CLEARED card. Honours the reduce-motion setting.
  final bool animate;
    ...
    return Row(
      ...
      children: [
        for (var i = 0; i < kMaxStars; i++)
          if (animate && !reduce && i < earned)
            _RevealStar(index: i, size: size)
          else
            Icon(
...
class _RevealStar extends StatelessWidget {
  const _RevealStar({required this.index, required this.size});
  final int index;
  final double size;

  @override
  Widget build(BuildContext context) {
    final delay = kStarRevealDelay + kStarRevealStep * index;
    final total = delay + const Duration(milliseconds: 420);
    final start = delay.inMilliseconds / total.inMilliseconds;
    return TweenAnimationBuilder<double>(
      key: ValueKey('star-reveal-$index'),
      tween: Tween(begin: 0, end: 1),
      duration: total,
      curve: Interval(start, 1, curve: Curves.elasticOut),
      builder: (_, k, child) => Opacity(
        opacity: k.clamp(0.0, 1.0),
        child: Transform.scale(scale: 0.2 + 0.8 * k, child: child),
      ),
      child: Icon(Icons.star_rounded, size: size, color: Palette.player),
    );
  }
}
