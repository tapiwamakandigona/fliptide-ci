...
abstract class FlipListener {
  ...
  /// The creature left its surface after a tap (gravity flipped).
  void onFlip();

  /// The creature met the other surface.
  void onLand();
}
...
class FlipGame extends FlameGame with TapCallbacks, KeyboardEvents {
  ...
  /// The corridor being played. Fixed for a Daily/Tide/code; The Deep swaps
  /// it per dive through [replaceCourse].
  Course course;
  ...
  /// The Deep: swap in a new corridor and start the next attempt at once,
  /// keeping this game (and its widget) alive so the HUD and music carry on.
  /// Honours the same post-death guard as [retry]. Returns false if refused.
  bool replaceCourse(Course course, {List<int> autoFlips = const []}) {
    if (sim.s.state == RunState.dead && _deathTimer <= kDeathTapGuardS) return false;
    if (sim.s.state == RunState.running) return false;
    this.course = course;
    _autoFlips = autoFlips.toSet();
    _ghostFlips = const {};
    lastDeathX = -1; // the marker belonged to the old corridor
    _reset();
    return true;
  }
  ...
  @visibleForTesting
  List<(double, double, Side, double)> get debugTrailSnapshot =>
      List.unmodifiable([
        for (final tr in _trail) (tr.x, tr.y, tr.side, tr.life),
      ]);
  ...
  // ---- render caches (0.4.0 perf pass) -----------------------------------
  // Paints that never change are built once; anything that depends on the
  // viewport (gradient shaders, stroke widths, spike paths) is rebuilt only
  // when the size changes. Before this the renderer allocated ~20 Paints,
  // 6 shaders and 3 Paths per frame.
  final Paint _slabPaint = Paint()..color = Palette.slab;
  ...
  final Paint _seamPaint = Paint()
    ..color = Palette.seam
    ..strokeWidth = 1;
  final Paint _farPaint = Paint()..color = Palette.pillar;
  final Paint _spikePaint = Paint()..color = Palette.spike;
  final Paint _spikeGlow = Paint()..color = Palette.spike.withValues(alpha: 0.10);
  final Paint _tipPaint = Paint()..color = Palette.spikeTip;
  final Paint _facetPaint = Paint()..color = Palette.spikeTip.withValues(alpha: 0.16);
  final Paint _blockLight = Paint()..color = Palette.slabLight;
  ...
  final Paint _topShadowPaint = Paint();
  final Paint _botShadowPaint = Paint();
  final Paint _finishWashPaint = Paint();
  ...
  Path _spikeGlowDown = Path();
  Path _facetUp = Path();
  Path _facetDown = Path();
  double _cacheW = -1, _cacheH = -1;

  void _rebuildCaches(double w, double vh, double t, double corridorTop, double corridorBot, double h) {
    _cacheW = w;
    _cacheH = vh;
    final interior = Rect.fromLTWH(0, corridorTop, w, h * t);
    _interiorPaint.shader = Gradient.linear(interior.topLeft, interior.bottomLeft, [Palette.corridorDeep, Palette.corridor, Palette.corridorDeep], [0, 0.5, 1]);
    final topSlab = Rect.fromLTRB(0, 0, w, corridorTop);
    final botSlab = Rect.fromLTRB(0, corridorBot, w, vh);
    if (topSlab.height > 0) {
      _topSlabPaint.shader = Gradient.linear(topSlab.bottomLeft, topSlab.topLeft, [Palette.slab, Palette.slabFar]);
    }
    if (botSlab.height > 0) {
      _botSlabPaint.shader = Gradient.linear(botSlab.topLeft, botSlab.bottomLeft, [Palette.slab, Palette.slabFar]);
    }
    ...
    _markerPaint.strokeWidth = math.max(2, t * 0.09);
    // Unit spike paths in tile space (x 0..t, base at y = 0); drawn translated.
    Path spike(double dir, double inset, double tipOver) => Path()
      ..moveTo(inset, 0)
      ..lineTo(t * 0.5, dir * t * 0.78 + dir * tipOver)
      ..lineTo(t - inset, 0)
      ..close();
    _spikeUp = spike(-1, t * 0.12, 0);
    _spikeDown = spike(1, t * 0.12, 0);
    _spikeGlowUp = spike(-1, t * 0.05, t * 0.10);
    _spikeGlowDown = spike(1, t * 0.05, t * 0.10);
    Path facet(double dir) => Path()
      ..moveTo(t * 0.12, 0)
      ..lineTo(t * 0.5, dir * t * 0.78)
      ..lineTo(t * 0.5, 0)
      ..close();
    _facetUp = facet(-1);
    _facetDown = facet(1);
  ...
  void update(double dt) {
    ...
    while (_acc >= kDt) {
      _acc -= kDt;
      // Trail age and sampling use the simulation clock, not paint count.
      // Paused repaints cannot grow it; 30/60/120 Hz cover the same distance.
      ...
      if (!sim.s.grounded && sim.s.frame % kTrailSampleEveryFrames == 0) {
        // Rendering interpolates one step behind sim.s. Sample that same
        // historical state so an afterimage never leads the visible player.
        final sample = _prev!;
        _trail.add(_Trail(sample.x, sample.y, sample.side, 1));
  ...
  void render(Canvas canvas) {
    ...
    if (w != _cacheW || vh != _cacheH) {
      _rebuildCaches(w, vh, t, corridorTop, corridorBot, h.toDouble());
    }
    ...
    final interior = Rect.fromLTWH(0, corridorTop, w, h * t);
    canvas.drawRect(interior, _interiorPaint);
    ...
    final shadowH = t * 0.35;
    canvas.drawRect(Rect.fromLTWH(0, corridorTop, w, shadowH), _topShadowPaint);
    canvas.drawRect(Rect.fromLTWH(0, corridorBot - shadowH, w, shadowH), _botShadowPaint);
    ...
    for (var cx = firstCol - 1; cx <= firstCol + tilesAcross + 2; cx++) {
      ...
      if (col.floorSpike && !col.isPit) {
        _drawSpike(canvas, x0, t, sy(col.floorH.toDouble()), -1);
      }
      if (col.ceilSpike) {
    ...
    if (fx > -t * 3 && fx < w + t) {
      ...
      // The wash moves with the camera, so its shader is per-frame by nature;
      // it only exists while the gate is on screen.
      _finishWashPaint.shader = Gradient.linear(wash.centerLeft, wash.centerRight, [Palette.finish.withValues(alpha: 0), Palette.finish.withValues(alpha: 0.10 * pulse)]);
      canvas.drawRect(wash, _finishWashPaint);
      final fp = _finishPaint..color = Palette.finish.withValues(alpha: pulse);
      for (var i = 0; i < h * 2; i++) {
        if (i.isEven) {
    ...
    if (lastDeathX >= 0 && sim.s.state == RunState.running) {
      ...
      if (mx > -t && mx < w + t) {
        final mp = _markerPaint..color = Palette.marker.withValues(alpha: 0.85);
    ...
    // Particles.
    final pp = _particlePaint;
    for (final pt in _particles) {
      pp.color = pt.color.withValues(alpha: pt.life.clamp(0.0, 1.0));
      final s = t * pt.size;
      canvas.save();
    ...
    if (_flash > 0) {
      ...
      _flashPaint.shader = Gradient.radial(
        Offset(w / 2, vh / 2),
        math.max(w, vh) * 0.75,
        [Palette.spike.withValues(alpha: 0), Palette.spike.withValues(alpha: 0), Palette.spike.withValues(alpha: 0.28 * k)],
        [0, 0.45, 1],
      );
      canvas.drawRect(Rect.fromLTWH(0, 0, w, vh), _flashPaint);
    }

    // Start hint.
    if (!_started) {
      canvas.drawRect(Rect.fromLTWH(0, 0, w, vh), _overlayPaint);
    }
  }
  ...
  void _drawSpike(Canvas canvas, double x0, double t, double base, double dir) {
    // dir −1 = points up from the floor, +1 = points down from the ceiling.
    // Cached unit paths, translated to the column (no per-spike Path objects).
    canvas.save();
    canvas.translate(x0, base);
    canvas.drawPath(dir < 0 ? _spikeGlowUp : _spikeGlowDown, _spikeGlow);
    canvas.drawPath(dir < 0 ? _spikeUp : _spikeDown, _spikePaint);
    canvas.drawPath(dir < 0 ? _facetUp : _facetDown, _facetPaint); // lit left facet
    final apex = dir * t * 0.78;
    canvas.drawCircle(Offset(t * 0.5, apex - dir * t * 0.06), t * 0.06, _tipPaint);
    canvas.restore();
  }
...
const int kTrailSampleEveryFrames = 2;
