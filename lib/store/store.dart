...
class Store {
  ...
  /// Lifetime gravity flips (all modes). Shown on the stats sheet.
  int get totalFlips => _p.getInt('flips') ?? 0;
  Future<void> addFlips(int n) => n <= 0 ? Future.value() : _p.setInt('flips', totalFlips + n);

  // ---- sound & feel (0.4.0) --------------------------------------------

  /// All three default ON; a fresh install hears the game.
  bool get musicOn => _p.getBool('pref.music') ?? true;
  bool get sfxOn => _p.getBool('pref.sfx') ?? true;
  bool get hapticsOn => _p.getBool('pref.haptics') ?? true;
  Future<void> setMusicOn(bool v) => _p.setBool('pref.music', v);
  Future<void> setSfxOn(bool v) => _p.setBool('pref.sfx', v);
  Future<void> setHapticsOn(bool v) => _p.setBool('pref.haptics', v);

  // ---- The Deep (endless, 0.4.0) ---------------------------------------

  /// Best depth in metres (= corridor columns survived). 0 = never dived.
  int get deepBest => _p.getInt('deep.best') ?? 0;
  int get deepRuns => _p.getInt('deep.runs') ?? 0;

  /// Record a dive. Returns true when it set a new best.
  Future<bool> recordDive(int metres) async {
    await _p.setInt('deep.runs', deepRuns + 1);
    if (metres > deepBest) {
      await _p.setInt('deep.best', metres);
      return true;
    }
    return false;
  }
